...
plugin.init = function(data, callback) {
  ...
  data.router.get(
    "/admin/plugins/composer-qiniu",
    data.middleware.admin.buildHeader,
    controllers.renderAdminPage
  );
  data.router.get(
    "/api/admin/plugins/composer-qiniu",
    controllers.renderAdminPage
  );
  data.router.get("/api/qiniu/token", function(req, res, next) {
    meta.settings.get("composer-qiniu", function(err, settings) {
      var mac = new qiniu.auth.digest.Mac(
        settings.qiniuAccessKey,
        settings.qiniuSecretKey
      );
      var scaleStyle = settings.qiniuScaleStyle;
      var options;
      if (scaleStyle) {
        var safeBase64 = qiniu.util.urlsafeBase64Encode(
          `${settings.qiniuBucket}:${req.query.key}`
        );
        options = {
          scope: settings.qiniuBucket,
          persistentOps: `${scaleStyle}|saveas/${safeBase64}`
          // "persistentOps": `imageView2/2/w/960|saveas/${safeBase64}`
        };
      } else {
        options = {
          scope: settings.qiniuBucket
...
plugin.appendConfig = function(config, callback) {
  meta.settings.get("composer-qiniu", function(err, settings) {
    ...
    delete settings.qiniuAccessKey;
    delete settings.qiniuSecretKey;
...
plugin.addPrefetchTags = function(hookData, callback) {
  ...
  hookData.links = hookData.links.concat(
    prefetch.map(function(path) {
      return {
        rel: "prefetch",
        href:
          nconf.get("relative_path") + path + "?" + meta.config["cache-buster"]
      };
    })
  );
    ...
    function(err, data) {
      ...
      if (req.query.cid) {
        discardRoute =
          ...
          "/category/" +
          validator.escape(String(req.query.cid));
      } else if (req.query.tid || req.query.pid) {
        if (data.topicData) {
          discardRoute =
            nconf.get("relative_path") + "/topic/" + data.topicData.slug;
        } else {
          return next();
        }
      }
      ...
      if (
        req.query.toPid &&
        parseInt(req.query.quoted, 10) === 1 &&
        data.postData
      ) {
        user.getUserField(data.postData.uid, "username", function(
          err,
          username
        ) {
      ...
      function ready() {
        ...
        callback(null, {
          ...
          templateData: {
            ...
            cid:
              parseInt(req.query.cid, 10) ||
              (data.topicData ? data.topicData.cid : null),
            action: action,
            ...
            allowTopicsThumbnail:
              parseInt(meta.config.allowTopicsThumbnail, 10) === 1 &&
              data.isMain,
            ...
            isEditing: isEditing,
            showHandleInput:
              parseInt(meta.config.allowGuestHandles, 10) === 1 &&
              (req.uid === 0 ||
                (isEditing && isGuestPost && (data.isAdmin || data.isMod))),
